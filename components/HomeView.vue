...
const loadTargetIssue = async () => {
  ...
  homePageSeo.value = {
    title: "無境界者雜誌",
    description: target.intro_home || "一個不以教會為本位的自由信仰論述平台",
    ogImage:
      target.cover_img ||
      "https://res.cloudinary.com/nonchurch2025/image/upload/topic.jpg",
  };
};

// ─── Computed：套用 Supabase translations ─────────────────────────

// 文章列表（標題、副標、作者、摘要、關鍵字）
...
watch(
  ...
  () => {
    ...
    const all = currentIssueContent.value
      .map((a) => a.keyword)
      .filter(Boolean)
      .join(",")
      .split(/[、,，]/)
...
// 期數資訊（標題、日期、intro_home、intro_cfp）
...
const groupArticles = (header) =>
  currentIssue.value?.content.filter((i) => i.section?.trim() === header) || [];
...
const currentIssueAuthors = computed(() => {
  ...
  let filtered = dbAuthors.value.filter((a) =>
    names.some((n) => n?.includes(a.name)),
  );
...
const handleAdminChange = () => {
  ...
  const base = route.path.startsWith("/admin") ? "/admin/home" : "/home";
...
<template>
  <div>
    <h1 class="page-main-title">
      ...
      {{
        route.params.issueNumber
          ? t.reviewTitle(route.params.issueNumber)
          : t.mainTitle
      }}
      <span class="emoji">🎉</span>
    </h1>
    ...
    <div v-else-if="!currentIssue" class="not-found-state">
      <h2>{{ t.notFound }}</h2>
      <p>{{ t.notFoundDesc }}</p>
      <NuxtLink to="/" class="btn">{{ t.backToLatest }}</NuxtLink>
    </div>
    ...
    <div v-else class="home-container">
      ...
      <section v-if="currentIssueAuthors.length" class="authors">
        ...
        <div class="author-container">
          <div v-for="a in currentIssueAuthors" :key="a.id" class="author">
            <NuxtLink :to="'/authors/' + a.name" :data-tooltip="t.viewAuthor">
              <img :src="a.author_image" :alt="a.displayName" />
            </NuxtLink>
            <h4>{{ a.displayName }}</h4>
          </div>
        </div>
      ...
      <section class="next-preview-submission">
        <div class="card-content">
          ...
          <NuxtLink to="/submit" class="btn">{{ t.cfp }}</NuxtLink>
        </div>
