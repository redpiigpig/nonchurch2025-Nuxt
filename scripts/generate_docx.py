...
class ProfessionalDocxGenerator:
    ...
    def _insert_figure_wordprocessing_group(
        ...
        """圖＋圖說：Word `wordprocessingGroup`（wpg:wgp），與參考稿 G:/我的雲端硬碟/資料/無境界者/雜誌/04-第四期/4-8跨越宗教藩籬的情誼.docx 相同類型。
    ...
    def _insert_info_card_wps_roundrect(self, paragraph, img_stream, title, link_items):
        """資訊卡：wps 圓角矩形外框 + 文字方塊內容（對照 G:/我的雲端硬碟/資料/無境界者/雜誌/01-第一期/1-11作為無教會主義的一種實踐？.docx 實測 OOXML）。"""
    ...
    def _apply_theme_image_frame(self, run):
        ...
        參考稿 G:/我的雲端硬碟/資料/無境界者/雜誌/07-第七期/7-1編輯室報告.docx 若可取得，可再微調 w／cmpd／adj）。"""
    ...
    def _drawingml_portrait_double_border_on_pic(self, pic_el):
        """在 pic:pic 的 spPr 上套用人物照雙線框（對照 G:/我的雲端硬碟/資料/無境界者/雜誌/07-第七期/7-5聆聽被遺忘的苦難.docx 浮動頭像）。"""
        from lxml import etree as lxml_etree
        PIC_NS = 'http://schemas.openxmlformats.org/drawingml/2006/picture'
        A_NS = 'http://schemas.openxmlformats.org/drawingml/2006/main'
