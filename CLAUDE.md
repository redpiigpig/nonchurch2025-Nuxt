...
| `chenwei-essay` | 張辰瑋丟素材／概念要你生成草稿（**全刊只有他是這樣接件**）。★2026-09-03 起改為：你備料、查證、擬大綱，**正文交給 Gemini 寫**。★2026-09-25 起：**分五關擬稿（複述→大綱→試寫一節→逐節→上架），每關先問過他才往下，不准一次寫完全篇** |
...
### 🚨 絕對禁止：擅自重新產生／覆蓋使用者正在編輯的交付檔（2026-09-19）

**事故**：使用者只要求「改 Word 裡的幾個字」，AI 卻重跑 `generate_docx.py` 產出整期 Word，
再覆蓋雲端硬碟（`G:\我的雲端硬碟\...\三校\`）上的同名檔，把使用者花一整天調好的
**圖片版面全部沖掉**，且舊檔本機無備份、只能靠 Drive 垃圾桶與版本記錄搶救。

**規則**：
- 使用者說「改字」就只改字。**改既有 docx 一律做定點 XML 手術**（zip 解開 → 改
  `word/document.xml` → 打包回去），不要重新產生整份檔案
- 重新產生整期／整篇 Word **必須先問過使用者**。就算他上一輪要過新檔，也不代表這一輪可以再產一次
- 任何要覆蓋雲端硬碟既有檔案的動作，**先複製一份原檔到暫存區備份**再寫入，並在回報中說明備份位置
- 產生的新檔預設**另存新檔名**（例如加 `_v2`），確定要取代才覆蓋
- 寫入前確認 Word 沒開著（看資料夾有沒有 `~$` 鎖檔、`tasklist | grep -i winword`）
- 檔案在 Google Drive 上時，救援路徑是：Drive 垃圾桶（30 天）、右鍵→檔案資訊→管理版本（30 天）。
  DriveFS 本機快取只留現行版本，救不了。Drive 檔案 ID 可從
  `%LOCALAPPDATA%\Google\DriveFS\<帳號>\metadata_sqlite_db` 的 `items` 表查
