...
| 階段 | 何時用 | 輸入 → 輸出 |
|---|---|---|
| **① Re-transcribe** | DB 還沒 content，或 queue txt 有 `# ⏳ 未完成` | YouTube URL → Whisper raw → Codex 清理 → DB |
...
| `tmp_sermon/<date>_clean.txt` | Codex 清理稿（intermediate） |

---

# Phase 1 — Re-transcribe（YouTube → DB）

Whisper raw 不夠好（無標點、簡體混雜、句子連在一起、Christian 名詞錯）— Codex 在 conversation 裡清，**不用 LLM API**（`.env` 沒 `ANTHROPIC_API_KEY`；Gemini cleanup 對神學名詞不穩）。
...
1. 使用者說「polish 2024-06-09」或 Codex 挑下一個。
...
逐篇校了 300 篇語音來源的講稿（錄音轉錄 263 篇＋2016／年議會／追思等語音整理稿 34 篇＋2013 生稿 3 篇）。作法：3 個 agent 一組接力，每批約 10 篇只「標記」不改寫；高信心直接套用，中信心交另一批 agent 看前後文獨立複核（Y／N／M），兩輪都同意才寫回；低信心與複核不採用的留給人工。寫回前對 DB 現值比對快照，不同就跳過不覆蓋。

- 已寫回：1,977 處誤聽、1,974 處簡轉繁機械錯字、3 處經課欄、4 項結構修正；原內容備份在 Drive `龐君華檔案\講道集\_備份\`。
- 待決清單（講者疑非龐君華 12 篇、放錯場次 6 組、生稿 18 篇、截斷 28 篇、經課欄錯位、643 條未套用建議）：Drive `龐君華檔案\講道集\講道集校對待決清單_2026-09-24.md`。

### 🚨 簡轉繁機械錯字（整庫曾有 2,000 處，已清為 0；新稿入庫前要掃）
「禰→祢」（稱呼上帝一律「祢」；禰是宗廟）、「隻有／隻是／不是隻→只」（但「那隻狗」「一隻羊」的量詞不動）、「紀唸／觀唸／顧唸→念」、「說明瞭／證明瞭／顯明瞭→了」（「瞭解」「明瞭」不動）、「裡麵→裡面」、「萬裡／峇裡→里」、「餘英時→余英時」、「徵戰／徵服→征」、「幹預／幹擾→干」、「複合／複活→復」、「松動／輕松→鬆」、「頭發→頭髮」（「舌頭發音」不動）。規則與排除條件見 know-graph-lab 那次的 `s2t_scan.py` 寫法：只收特定搭配下必錯的組合。

### 追加誤聽對照（語音稿常見）
慧眾→會眾、祖日→主日、上祖→上主、將領節→將臨節、聖靈嘉年節→聖靈降臨節、基督威王→基督為王、十里文→啟應文、使毒性金→使徒信經、以賽亞（指基督時）→彌賽亞、藏在我裡面→常在我裡面（約15）、馬門→瑪門、稅令→稅吏、多瑪→多馬、尼哥德慕→尼哥底母、三讀三島→三讀三禱、旅程碑→里程碑、九月的經課→舊約的經課。和合本作**撒瑪利亞**（不是撒馬利亞）；「變像主日」是教會通用寫法，不要改成「變相」。

### 查核時一併要看的（不是錯字、但比錯字嚴重）
1. **講者不是龐君華**：客座、神學生、別的會督的講道被掛成龐牧師（看自述經歷：出身、任職年份、「會督也在這裡」）。
2. **內文放錯場次**：開頭常自述「今天是某某主日」，拿來對 occasion；2006 年 6 月曾有同一篇連掛三週。
3. **經課欄錯位一週**：2013 年起多篇 scripture_ref 抄到下一主日的經課；判準是正文實際讀、講的經文。
...
## 影音逐字稿（pong_media）要跟著講道內文一起改（2026-09-24）

- 講道錄音那批 `pong_media.transcript` 與 `pong_sermons.content` 原本是同一份文字；改講道內文時要同步改影音逐字稿（2026-09-24 同步了 209 筆），否則兩邊會分岔。`sermon_retranscribe_commit.py` 已兩邊一起寫。
- 獨立演講／講座／訪談 16 篇也照講道集同一套流程校過（逐條標記→中信心第二輪複核→寫回，242 處誤聽＋117 處簡轉繁），備份在 Drive `訪談與演講\_備份\`。演講裡常混入主持人、評論者、提問者的發言，講者標籤要分清楚。
- 刪講道列時，檢查它掛的 `media_id` 與同日的分段影音（Part 1／Part 2）要不要一起刪。
