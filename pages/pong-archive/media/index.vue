        ...
        >
          ...
          <div class="mi-card-body">
            <div class="mi-card-meta">
              <span v-if="item.source" class="mi-source">{{
                item.source
              }}</span>
              <span v-if="item.broadcast_date" class="mi-date">{{
                formatDate(item.broadcast_date)
              }}</span>
            </div>
            <h3 class="mi-card-title">{{ item.title }}</h3>
            <p v-if="item.description" class="mi-card-desc">
              {{ item.description }}
            </p>
      ...
      <div v-if="shorts.length" class="mi-shorts-grid">
        ...
        >
          <div
            ...
            :style="
              item.thumbnail_url || item.youtube_id
                ? {
...
const { data: items } = await useAsyncData("pong-media-list-v2", async () => {
  const { data, error } = await supabase
    .from("pong_media")
    .select(
      "id, title, title_en, source, program_name, interviewer, media_type, platform, youtube_id, broadcast_date, description, thumbnail_url, tags",
    )
