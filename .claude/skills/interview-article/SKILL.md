...
| Class | 用途 |
|-------|------|
| `class="no-indent"` | **只用在訪談主體 Q&A 的「問」與「答」首段**（也就是 `<strong>丞譽：</strong>` / `<strong>師母：</strong>` 這類段落） |
| 不加 class | 一般段落，**包括訪談簡介、受訪者簡介、作品/組織簡介、訪談後記的所有段落**（CSS 預設縮排兩字） |

⚠️ **早期（4-x ~ 9-7 INSERT 後第一版）的訪問把訪談簡介首段也寫成 `no-indent`，這是錯的。** 中文書面排版「章節首段不縮排」的慣例不適用於這類雜誌訪問——使用者明確要求訪談簡介、受訪者簡介、訪談後記等敘事段落「全部都縮排」。Q&A 主體的問答首段不縮排是為了讓「問—答—問—答」的視覺節奏更清楚，那是 Q&A 特有的需要。
